...
import { PgConnection, ConnectionNotFoundError, TransactionNotFoundError } from '@/infra/repository/postgresql/helpers'
...
describe('PgConnection', () => {
  ...
  it('should get repository from transaction', async () => {
    await sut.connect()
    await sut.openTransaction()
    const repository = sut.getRepository(PostgreSQLUser)

    expect(getRepositorySpy).toHaveBeenCalledWith(PostgreSQLUser)
  ...
  it('should return ConnectionNotFoundError on getRepository if connection is not found', async () => {
    ...
    expect(() => sut.getRepository(PostgreSQLUser)).toThrow(new ConnectionNotFoundError())
  })
})
