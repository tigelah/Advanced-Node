...
describe('JwtTokenHandler', () => {
  ...
  describe('generateToken', () => {
    ...
    it('should call sign with correct params', async () => {
      await sut.generate({ key: key, expirationInMs })
      expect(fakeJwt.sign).toHaveBeenCalledWith({ key: key }, secret, { expiresIn: 1 })
      expect(fakeJwt.sign).toHaveBeenCalledTimes(1)
    })

    it('should return a token', async () => {
      const generatedToken = await sut.generate({ key: key, expirationInMs })
    ...
    it('should rethrow if sign throws', async () => {
      ...
      const promise = sut.generate({ key: key, expirationInMs })
